...
**Each card is a `Panel` (150 × 180 px) containing:**

1. A `PictureBox` (96 × 96, centered): shows the avatar image if `LocalAvatarPath` exists and is readable; otherwise shows a generated initial badge (first letter of the display name, on a coloured circle derived from the name's hash).
...
- The **Done** button (`"Done"`, 140 × 48 px, bottom-right) is disabled until a card is selected.
- Pressing Done sets `DialogResult = OK` and closes the form.

**Accessibility requirement:**  
Button and card sizes must be large enough for reliable eye-gaze activation. Minimum touch/gaze target: 44 × 44 px. Cards at 150 × 180 px meet this requirement. Do not make them smaller.
...
- [ ] Fallback avatar is a coloured circle with the first initial (not a generic icon)
- [ ] UI background is dark (`#202124`)
- [ ] Cards are at least 44 × 44 px (150 × 180 px recommended)
