...
internal static class NavigationActions
{
    ...
    public static string BuildFullscreenDebugSetupScript()
    {
        return """
            (() => {
                if (!window.__ytFsDebug) {
                    window.__ytFsDebug = {
                        events: [],
                ...
                if (!window.__ytFsDebug.listenerInstalled) {
                    window.__ytFsDebug.listenerInstalled = true;

                    document.addEventListener('fullscreenchange', () => {
                        const el = document.fullscreenElement;
                        window.__ytFsDebug.events.push({
                            type: 'fullscreenchange',
                            hasFullscreenElement: !!el,
                            fullscreenTag: el ? el.tagName : null,
                            ts: Date.now()
                        });
                    }, true);

                    document.addEventListener('fullscreenerror', (ev) => {
                        const msg = (ev && ev.message) ? ev.message : 'fullscreenerror';
                        window.__ytFsDebug.errors.push({ type: 'fullscreenerror', message: msg, ts: Date.now() });
                    }, true);

                    window.addEventListener('error', (ev) => {
                        const msg = (ev && ev.message) ? ev.message : '';
                        if (msg && msg.includes('NotAllowedError')) {
                            window.__ytFsDebug.errors.push({ type: 'window.error', message: msg, ts: Date.now() });
                        }
                    }, true);
                }

                return 'fullscreen-debug-listeners-ready';
            })()
            """;
    }

    public static string BuildFullscreenStateSnapshotScript(string stage)
    {
        var encodedStage = QuoteJsString(stage ?? string.Empty);
        return """
            (() => {
            ...
            """.Replace("__STAGE__", encodedStage, StringComparison.Ordinal);
    }

    public static string BuildFullscreenEventSummaryScript()
    {
        return """
            (() => {
                const d = window.__ytFsDebug || { events: [], errors: [] };
                return JSON.stringify({
                    eventCount: d.events.length,
                    lastEvent: d.events.length ? d.events[d.events.length - 1] : null,
                    errorCount: d.errors.length,
                    lastError: d.errors.length ? d.errors[d.errors.length - 1] : null
                });
            })()
            """;
    }
